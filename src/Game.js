...
export class Game extends React.Component {
    ...
    handleClick(coordinates) {
        ...
        if (clickedSquare !== null) {
            ...
            // Set active piece
            let movesData = this.ReactCheckers.getMoves(boardState, coordinates, false);
        ...
        if (this.state.moves.length > 0) {
            const postMoveState = this.ReactCheckers.playerMove(coordinates, this.state);
    ...
    redo(){
        /*
        if (flag) {
            const backStep = parseInt(redoSteps, 10)-1 ;
            console.log("backstep : ", backStep);
            console.log("redoSteps :" , redoSteps);
            if (backStep < 0) {
                return;
            }
            const unsetHistory = redoSteps.slice(0, backStep+1);
            this.setState({
                history: unsetHistory,
                activePiece: null,
                moves: [],
    ...
    render() {
        ...
        switch (this.state.winner) {
            case 'player1pieces':
                gameStatus = 'First Player Wins!';
                break;
            case 'player2pieces':
                gameStatus = 'Second Player Wins!';
                break;
            case 'player1moves':
                gameStatus = 'No moves left -First Player Wins!';
                break;
            case 'player2moves':
                gameStatus = 'No moves left -Second Player Wins!';
                break;
            default:
                gameStatus = currentState.currentPlayer === true ? 'First Player' : 'Second Player'; 
                break;
        }
        ...
        if (this.state.players === null) {
            return(
                <Router history={browserHistory} basename={'react-checkers'} >
                    ...
                    <div className="enterance-screen">Basic Classical Checkers Game</div>
                        <div className="players">
                            
                            <div className="two-player" onClick={()=> this.setPlayers(2) }>Start Game</div>
                            
                        </div>
                        <div className="enterance-screen by">By : Nasim Daghash</div>
                    </div>
                </Router>
            )
        }
            ...
            <Router history={browserHistory} basename={'react-checkers'} >
                <div className="reactCheckers">
                    ...
                    <div className="control-buttons">
                        ...
                        <button className={redoClass} onClick={()=>this.redo()}>Redo</button>
                        <button className={restartClass} onClick={()=>this.startAgain()}>Start Game</button>
                    </div>
                </div>
            </Router>
        );
